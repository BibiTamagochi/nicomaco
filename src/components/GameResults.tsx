...
export const GameResults = ({ score, companyName, onRestart }: GameResultsProps) => {
  const getPerformanceLevel = (score: number) => {
    if (score === 50) return { level: 'Expert Sustentável', icon: '🏆', color: 'text-yellow-500', stars: 5 };
    if (score === 40) return { level: 'Empresário Verde', icon: '🥇', color: 'text-green-500', stars: 4 };
    if (score === 30) return { level: 'Consciente Ambiental', icon: '🥈', color: 'text-blue-500', stars: 3 };
    if (score === 20) return { level: 'Iniciante Sustentável', icon: '🥉', color: 'text-orange-500', stars: 2 };
    if (score === 10) return { level: 'Aprendiz Verde', icon: '🌱', color: 'text-green-400', stars: 1 };
    return { level: 'Precisa Melhorar', icon: '📚', color: 'text-gray-500', stars: 0 };
  };

  const performance = getPerformanceLevel(score);
  const stars = Math.max(0, Math.min(5, Math.floor(score / 10))); // Estrelas = pontos / 10
        ...
        <CardContent className="space-y-6">
          ...
          <div className="bg-muted/30 rounded-lg p-4 text-left">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p className="font-bold text-lg">{Math.max(0, score)} pts</p>
              </div>
              <div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-4">
            ...
            <Button
              ...
              size="lg"
              onClick={() => {
                const text = `🌿 Acabei de completar o Nicômaco Quiz com ${stars} estrelas ⭐ (${Math.max(0, score)} pontos) como ${performance.level}! 
